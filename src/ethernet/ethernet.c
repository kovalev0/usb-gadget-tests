...
// Maximum length for UDC driver and device names
#define UDC_NAME_LENGTH_MAX 128

// Structure for initializing the USB raw gadget
struct usb_raw_init {
...
// Enumeration of USB raw gadget event types
...
// Structure for USB raw gadget events
...
// Structure for USB endpoint I/O operations
...
bool ep0_request(int fd, struct usb_raw_control_event *event,
	...
	case USB_TYPE_VENDOR:
		switch (event->ctrl.bRequest) {
		case 0x05: // REQ_GET_REGS
			if (event->ctrl.wValue == 0x012e) {
				io->data[0] = 0x20;
				io->inner.length = 1;
				return true;
			}
			else if (event->ctrl.wValue == 0x0120) {
				io->data[0] = 0x05;
				io->inner.length = event->ctrl.wLength;
