...
//
// Emulates a Cypress USB Thermometer device (VID: 0x04b4, PID: 0x0002).
...
// SET_CONFIGURATION) and vendor requests READ_RAM (0x02), WRITE_RAM (0x03),
// READ_PORT (0x04), WRITE_PORT (0x05).
//
// The emulator exercises the cytherm driver via sysfs attributes:
// brightness, temp, button, port0, port1.
//
...
void log_control_request(struct usb_ctrlrequest *ctrl) {
	...
	case USB_TYPE_VENDOR:
		...
		printf("  req = 0x%x, value = 0x%x, index = 0x%x\n",
...
void *test_thread(void *arg) {
	...
	{
		char attr_path[1024];
		snprintf(attr_path, sizeof(attr_path), "%s/temp", dev_path);
		int waited = 0;
	...
	// temp_show: READ_RAM(TEMP=0x33) + READ_RAM(SIGN=0x34)
	if (sysfs_read(dev_path, "temp", buf, sizeof(buf)) > 0)
		printf("[TEST] temp = %s\n", buf);

	// button_show: READ_RAM(BUTTON=0x7a)
	if (sysfs_read(dev_path, "button", buf, sizeof(buf)) > 0)
		printf("[TEST] button = %s\n", buf);

	// brightness_show: reads cached cytherm->brightness (no USB transfer)
	if (sysfs_read(dev_path, "brightness", buf, sizeof(buf)) > 0)
		printf("[TEST] brightness = %s\n", buf);

	// brightness_store: WRITE_RAM(BRIGHTNESS=0x2c) + WRITE_RAM(BRIGHTNESS_SEM=0x2b)
	printf("[TEST] Setting brightness = 128\n");
	if (sysfs_write(dev_path, "brightness", "128") < 0)
		printf("[TEST] ERR: write brightness: %s\n",
							strerror(errno));

	if (sysfs_read(dev_path, "brightness", buf, sizeof(buf)) > 0)
		printf("[TEST] brightness after write = %s\n", buf);

	// port0_show: READ_PORT(addr=0)
	if (sysfs_read(dev_path, "port0", buf, sizeof(buf)) > 0)
		printf("[TEST] port0 = %s\n", buf);

	// port0_store: WRITE_PORT(addr=0, data=42)
	printf("[TEST] Writing port0 = 42\n");
	if (sysfs_write(dev_path, "port0", "42") < 0)
		printf("[TEST] ERR: write port0: %s\n",
							strerror(errno));

	// port1_show: READ_PORT(addr=1)
	if (sysfs_read(dev_path, "port1", buf, sizeof(buf)) > 0)
		printf("[TEST] port1 = %s\n", buf);

	// port1_store: WRITE_PORT(addr=1, data=255)
...
#define USB_VENDOR		0x04b4	// Cypress Semiconductor
#define USB_PRODUCT		0x0002	// CY7C63xxx thermometer

// Vendor command codes (mirror of driver defines)
#define READ_RAM		0x02
#define WRITE_RAM		0x03
#define READ_PORT		0x04
#define WRITE_PORT		0x05

// RAM locations mirrored by the emulator
...
// Emulated RAM: 256 bytes, pre-filled with meaningful defaults
...
bool ep0_request(int fd, struct usb_raw_control_event *event,
	...
	case USB_TYPE_VENDOR: {
		...
		uint8_t value = event->ctrl.wValue & 0xff; // address
		uint8_t index = event->ctrl.wIndex & 0xff; // data (for writes)

		memset(io->data, 0, CYTHERM_BUF_SIZE);
		io->inner.length = CYTHERM_BUF_SIZE;

		switch (req) {
		case READ_RAM:
			io->data[1] = ram[value];
			// printf("ep0: READ_RAM[0x%02x] -> 0x%02x\n",
			// 				value, ram[value]);
			break;
		case WRITE_RAM:
			ram[value] = index;
			// printf("ep0: WRITE_RAM[0x%02x] <- 0x%02x\n",
			// 				value, index);
			break;
		case READ_PORT:
			io->data[1] = port[value & 0x01];
			// printf("ep0: READ_PORT[%d] -> 0x%02x\n",
			// 			value & 1, io->data[1]);
			break;
		case WRITE_PORT:
			port[value & 0x01] = index;
			// printf("ep0: WRITE_PORT[%d] <- 0x%02x\n",
			// 				value & 1, index);
			break;
		default:
...
int main(int argc, char **argv) {
	...
	// Pre-fill RAM with realistic defaults:
	//   TEMP=0x33: 0x2A = 42 -> +21.0 C
	//   SIGN=0x34: 0    = positive
	//   BUTTON=0x7a: 0  = not pressed
	//   BRIGHTNESS=0x2c: 0xFF (matches driver default)
	ram[TEMP]       = 0x2A;
	ram[SIGN]       = 0x00;
	ram[BUTTON]     = 0x00;
	ram[BRIGHTNESS] = 0xFF;
