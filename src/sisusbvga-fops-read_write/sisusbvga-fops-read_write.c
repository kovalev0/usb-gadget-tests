...
// - 8MB VRAM emulation with SDR 1ch/1rank (1x) configuration
...
//
// FILE OPERATIONS TESTING: read/write operations with address alignment (40 tests)
// Tests device file operations interface focusing on:
// - I/O port read/write operations (1, 2, 4 byte sizes)
// - VRAM read/write operations at aligned addresses (1, 2, 3, 4 bytes)
// - VRAM read/write operations at unaligned addresses (WORD, 24-BIT, LONG)
// - PCI configuration space access
// - lseek operations (SEEK_SET, SEEK_CUR, SEEK_END)
//
...
// Bulk write to VRAM - for large data transfers
...
void test_device_file_operations(void) {
	char devpath[512];
	uint8_t buf8;
	uint16_t buf16;
	uint32_t buf32;
	...
	printf("\n[TEST 1] Read I/O port (1 byte)\n");
	off_t offset = SISUSB_PCI_PSEUDO_IOPORTBASE + 0x44;
	rv = lseek(devfd, offset, SEEK_SET);
	if (rv < 0) {
		printf("[TEST 1] ERROR: lseek failed: %s\n", strerror(errno));
	} else {
		rv = read(devfd, &buf8, 1);
		if (rv < 0) {
			printf("[TEST 1] ERROR: read failed: %s\n", strerror(errno));
		} else {
			printf("[TEST 1] OK: Read 1 byte from I/O port 0x44, value = 0x%02x\n", buf8);
		}
	}

	// Test 2: Read I/O port (2 bytes)
	printf("\n[TEST 2] Read I/O port (2 bytes)\n");
	offset = SISUSB_PCI_PSEUDO_IOPORTBASE + 0x44;
	rv = lseek(devfd, offset, SEEK_SET);
	if (rv >= 0) {
		rv = read(devfd, &buf16, 2);
		if (rv >= 0) {
			printf("[TEST 2] OK: Read 2 bytes from I/O port 0x44, value = 0x%04x\n", buf16);
		}
	}
	...
	rv = lseek(devfd, offset, SEEK_SET);
	if (rv >= 0) {
		rv = read(devfd, &buf32, 4);
		if (rv >= 0) {
			printf("[TEST 3] OK: Read 4 bytes from I/O port 0x44, value = 0x%08x\n", buf32);
		}
	}

	// Test 4: Write I/O port (1 byte)
	printf("\n[TEST 4] Write I/O port (1 byte)\n");
	offset = SISUSB_PCI_PSEUDO_IOPORTBASE + 0x45;
	buf8 = 0xAB;
	rv = lseek(devfd, offset, SEEK_SET);
	if (rv >= 0) {
		rv = write(devfd, &buf8, 1);
		if (rv >= 0) {
			printf("[TEST 4] OK: Wrote 1 byte to I/O port 0x45, value = 0x%02x\n", buf8);
		}
	}

	// Test 5: Write I/O port (2 bytes)
	printf("\n[TEST 5] Write I/O port (2 bytes)\n");
	offset = SISUSB_PCI_PSEUDO_IOPORTBASE + 0x46;
	buf16 = 0xABCD;
	rv = lseek(devfd, offset, SEEK_SET);
	if (rv >= 0) {
		rv = write(devfd, &buf16, 2);
		if (rv >= 0) {
			printf("[TEST 5] OK: Wrote 2 bytes to I/O port 0x46, value = 0x%04x\n", buf16);
		}
	}

	// Test 6: Write I/O port (4 bytes)
	printf("\n[TEST 6] Write I/O port (4 bytes)\n");
	offset = SISUSB_PCI_PSEUDO_IOPORTBASE + 0x48;
	buf32 = 0xDEADBEEF;
	rv = lseek(devfd, offset, SEEK_SET);
	if (rv >= 0) {
		rv = write(devfd, &buf32, 4);
		if (rv >= 0) {
			printf("[TEST 6] OK: Wrote 4 bytes to I/O port 0x48, value = 0x%08x\n", buf32);
		}
	}

	// Test 7: Read VRAM
	printf("\n[TEST 7] Read VRAM\n");
	offset = SISUSB_PCI_PSEUDO_MEMBASE + 0x1000;
	uint8_t vram_buf[16];
	rv = lseek(devfd, offset, SEEK_SET);
	if (rv >= 0) {
		rv = read(devfd, vram_buf, sizeof(vram_buf));
		if (rv >= 0) {
			printf("[TEST 7] OK: Read %d bytes from VRAM offset 0x1000\n", rv);
		}
	}

	// Test 8: Write VRAM
	printf("\n[TEST 8] Write VRAM\n");
	offset = SISUSB_PCI_PSEUDO_MEMBASE + 0x2000;
	...
	rv = lseek(devfd, offset, SEEK_SET);
	if (rv >= 0) {
		rv = write(devfd, test_pattern, sizeof(test_pattern));
		if (rv >= 0) {
			printf("[TEST 8] OK: Wrote %d bytes to VRAM offset 0x2000\n", rv);
		}
	}

	// Test 9: Write PCI config
	printf("\n[TEST 9] Write PCI config\n");
	offset = SISUSB_PCI_PSEUDO_PCIBASE + 0x10;
	buf32 = 0x12345678;
	rv = lseek(devfd, offset, SEEK_SET);
	if (rv >= 0) {
		rv = write(devfd, &buf32, 4);
		if (rv >= 0) {
			printf("[TEST 9] OK: Wrote PCI config offset 0x10, value = 0x%08x\n", buf32);
		}
	}
	...
	if (rv >= 0) {
		rv = read(devfd, &buf32, 4);
		if (rv >= 0) {
			printf("[TEST 10] OK: Read PCI config offset 0x10, value = 0x%08x\n", buf32);
		}
	}
	...
	rv = lseek(devfd, offset, SEEK_SET);
	if (rv >= 0) {
		rv = lseek(devfd, 0x100, SEEK_CUR);
		if (rv >= 0) {
			printf("[TEST 11] OK: lseek SEEK_CUR, new offset = 0x%lx\n", (long)rv);
		}
	}

	// Test 12: lseek with SEEK_END
	printf("\n[TEST 12] lseek with SEEK_END\n");
	rv = lseek(devfd, 0, SEEK_END);
	if (rv >= 0) {
		printf("[TEST 12] OK: lseek SEEK_END, offset = 0x%lx\n", (long)rv);
	}

	// Test 13: Write VRAM (1 byte) - triggers sisusb_write_memio_byte path
	printf("\n[TEST 13] Write VRAM (1 byte)\n");
	offset = SISUSB_PCI_PSEUDO_MEMBASE + 0x3000;
	buf8 = 0xAA;
	rv = lseek(devfd, offset, SEEK_SET);
	if (rv >= 0) {
		rv = write(devfd, &buf8, 1);
		if (rv >= 0) {
			printf("[TEST 13] OK: Wrote 1 byte to VRAM offset 0x3000, value = 0x%02x\n", buf8);
		}
	}

	// Test 14: Write VRAM (2 bytes) - triggers sisusb_write_memio_word path
	printf("\n[TEST 14] Write VRAM (2 bytes)\n");
	offset = SISUSB_PCI_PSEUDO_MEMBASE + 0x3010;
	buf16 = 0xBEEF;
	rv = lseek(devfd, offset, SEEK_SET);
	if (rv >= 0) {
		rv = write(devfd, &buf16, 2);
		if (rv >= 0) {
			printf("[TEST 14] OK: Wrote 2 bytes to VRAM offset 0x3010, value = 0x%04x\n", buf16);
		}
	}

	// Test 15: Write VRAM (3 bytes) - triggers sisusb_write_memio_24bit path
	printf("\n[TEST 15] Write VRAM (3 bytes)\n");
	offset = SISUSB_PCI_PSEUDO_MEMBASE + 0x3020;
	uint8_t buf3[3] = {0x12, 0x34, 0x56};
	rv = lseek(devfd, offset, SEEK_SET);
	if (rv >= 0) {
		rv = write(devfd, buf3, 3);
		if (rv >= 0) {
			printf("[TEST 15] OK: Wrote 3 bytes to VRAM offset 0x3020\n");
		}
	}

	// Test 16: Write VRAM (4 bytes) - triggers sisusb_write_memio_long path
	printf("\n[TEST 16] Write VRAM (4 bytes)\n");
	offset = SISUSB_PCI_PSEUDO_MEMBASE + 0x3030;
	buf32 = 0xDEADC0DE;
	rv = lseek(devfd, offset, SEEK_SET);
	if (rv >= 0) {
		rv = write(devfd, &buf32, 4);
		if (rv >= 0) {
			printf("[TEST 16] OK: Wrote 4 bytes to VRAM offset 0x3030, value = 0x%08x\n", buf32);
		}
	}

	// Test 17: Read VRAM (1 byte) - triggers sisusb_read_memio_byte path
	...
	if (rv >= 0) {
		rv = read(devfd, &buf8, 1);
		if (rv >= 0) {
			printf("[TEST 17] OK: Read 1 byte from VRAM offset 0x3000, value = 0x%02x\n", buf8);
		}
	}

	// Test 18: Read VRAM (2 bytes) - triggers sisusb_read_memio_word path
	...
	if (rv >= 0) {
		rv = read(devfd, &buf16, 2);
		if (rv >= 0) {
			printf("[TEST 18] OK: Read 2 bytes from VRAM offset 0x3010, value = 0x%04x\n", buf16);
		}
	}

	// Test 19: Read VRAM (3 bytes) - triggers sisusb_read_memio_24bit path
	printf("\n[TEST 19] Read VRAM (3 bytes)\n");
	offset = SISUSB_PCI_PSEUDO_MEMBASE + 0x3020;
	uint8_t read3[3];
	rv = lseek(devfd, offset, SEEK_SET);
	if (rv >= 0) {
		rv = read(devfd, read3, 3);
		if (rv >= 0) {
			printf("[TEST 19] OK: Read 3 bytes from VRAM offset 0x3020, values = 0x%02x 0x%02x 0x%02x\n",
				read3[0], read3[1], read3[2]);
		}
	}


	// Test 20: Read with invalid count (5 bytes from I/O) - should fail
	printf("\n[TEST 20] Read I/O port (invalid 5 bytes)\n");
	offset = SISUSB_PCI_PSEUDO_IOPORTBASE + 0x44;
	uint8_t buf5[5];
	rv = lseek(devfd, offset, SEEK_SET);
	if (rv >= 0) {
		rv = read(devfd, buf5, 5);
		if (rv < 0) {
			printf("[TEST 20] OK: Invalid read size correctly rejected\n");
		} else {
			printf("[TEST 20] UNEXPECTED: Read of 5 bytes should have failed\n");
		}
	}

	// Test 21: Write with invalid count (5 bytes to I/O) - should fail
	printf("\n[TEST 21] Write I/O port (invalid 5 bytes)\n");
	offset = SISUSB_PCI_PSEUDO_IOPORTBASE + 0x44;
	rv = lseek(devfd, offset, SEEK_SET);
	if (rv >= 0) {
		rv = write(devfd, buf5, 5);
		if (rv < 0) {
			printf("[TEST 21] OK: Invalid write size correctly rejected\n");
		} else {
			printf("[TEST 21] UNEXPECTED: Write of 5 bytes should have failed\n");
		}
	}

	// Test 22: Write to PCI config with invalid size - should fail
	...
	if (rv >= 0) {
		rv = write(devfd, &buf16, 2);
		if (rv < 0) {
			printf("[TEST 22] OK: Invalid PCI config write size correctly rejected\n");
		} else {
			printf("[TEST 22] UNEXPECTED: PCI config write of 2 bytes should have failed\n");
		}
	}

	// =====================================================================
	// GROUP A: WORD (2 bytes) Write - Unaligned addresses (case 1, 2, 3)
	// =====================================================================

	printf("\n========== GROUP A: WORD Write (Unaligned) ==========\n");

	// TEST 23: Write WORD to addr+1 (case 1 in sisusb_write_memio_word)
	printf("\n[TEST 23] Write VRAM WORD (2 bytes) at offset +1\n");
	offset = SISUSB_PCI_PSEUDO_MEMBASE + 0x4001;  // Unaligned: addr & 3 == 1
	buf16 = 0xABCD;
	rv = lseek(devfd, offset, SEEK_SET);
	if (rv >= 0) {
		rv = write(devfd, &buf16, 2);
		if (rv >= 0) {
			printf("[TEST 23] OK: Wrote 2 bytes at +1 offset, value = 0x%04x\n", buf16);
		}
	}

	// TEST 24: Write WORD to addr+2 (case 2 in sisusb_write_memio_word)
	printf("\n[TEST 24] Write VRAM WORD (2 bytes) at offset +2\n");
	offset = SISUSB_PCI_PSEUDO_MEMBASE + 0x4002;  // Unaligned: addr & 3 == 2
	buf16 = 0x1234;
	rv = lseek(devfd, offset, SEEK_SET);
	if (rv >= 0) {
		rv = write(devfd, &buf16, 2);
		if (rv >= 0) {
			printf("[TEST 24] OK: Wrote 2 bytes at +2 offset, value = 0x%04x\n", buf16);
		}
	}

	// TEST 25: Write WORD to addr+3 (case 3 in sisusb_write_memio_word)
	printf("\n[TEST 25] Write VRAM WORD (2 bytes) at offset +3\n");
	offset = SISUSB_PCI_PSEUDO_MEMBASE + 0x4003;  // Unaligned: addr & 3 == 3
	buf16 = 0x5678;
	rv = lseek(devfd, offset, SEEK_SET);
	if (rv >= 0) {
		rv = write(devfd, &buf16, 2);
		if (rv >= 0) {
			printf("[TEST 25] OK: Wrote 2 bytes at +3 offset, value = 0x%04x\n", buf16);
		}
	}

	// =====================================================================
	// GROUP B: 24-BIT (3 bytes) Write - Unaligned addresses (case 1, 2, 3)
	// =====================================================================

	printf("\n========== GROUP B: 24-BIT Write (Unaligned) ==========\n");

	// TEST 26: Write 24-BIT to addr+1 (case 1 in sisusb_write_memio_24bit)
	printf("\n[TEST 26] Write VRAM 24-BIT (3 bytes) at offset +1\n");
	offset = SISUSB_PCI_PSEUDO_MEMBASE + 0x5001;  // Unaligned: addr & 3 == 1
	uint8_t buf3_test1[3] = {0xAA, 0xBB, 0xCC};
	rv = lseek(devfd, offset, SEEK_SET);
	if (rv >= 0) {
		rv = write(devfd, buf3_test1, 3);
		if (rv >= 0) {
			printf("[TEST 26] OK: Wrote 3 bytes at +1 offset\n");
		}
	}

	// TEST 27: Write 24-BIT to addr+2 (case 2 in sisusb_write_memio_24bit)
	printf("\n[TEST 27] Write VRAM 24-BIT (3 bytes) at offset +2\n");
	offset = SISUSB_PCI_PSEUDO_MEMBASE + 0x5002;  // Unaligned: addr & 3 == 2
	uint8_t buf3_test2[3] = {0x11, 0x22, 0x33};
	rv = lseek(devfd, offset, SEEK_SET);
	if (rv >= 0) {
		rv = write(devfd, buf3_test2, 3);
		if (rv >= 0) {
			printf("[TEST 27] OK: Wrote 3 bytes at +2 offset\n");
		}
	}

	// TEST 28: Write 24-BIT to addr+3 (case 3 in sisusb_write_memio_24bit)
	printf("\n[TEST 28] Write VRAM 24-BIT (3 bytes) at offset +3\n");
	offset = SISUSB_PCI_PSEUDO_MEMBASE + 0x5003;  // Unaligned: addr & 3 == 3
	uint8_t buf3_test3[3] = {0x44, 0x55, 0x66};
	rv = lseek(devfd, offset, SEEK_SET);
	if (rv >= 0) {
		rv = write(devfd, buf3_test3, 3);
		if (rv >= 0) {
			printf("[TEST 28] OK: Wrote 3 bytes at +3 offset\n");
		}
	}

	// =====================================================================
	// GROUP C: LONG (4 bytes) Write - Unaligned addresses (case 1, 2, 3)
	// =====================================================================

	printf("\n========== GROUP C: LONG Write (Unaligned) ==========\n");

	// TEST 29: Write LONG to addr+1 (case 1 in sisusb_write_memio_long)
	printf("\n[TEST 29] Write VRAM LONG (4 bytes) at offset +1\n");
	offset = SISUSB_PCI_PSEUDO_MEMBASE + 0x6001;  // Unaligned: addr & 3 == 1
	buf32 = 0xDEADBEEF;
	rv = lseek(devfd, offset, SEEK_SET);
	if (rv >= 0) {
		rv = write(devfd, &buf32, 4);
		if (rv >= 0) {
			printf("[TEST 29] OK: Wrote 4 bytes at +1 offset, value = 0x%08x\n", buf32);
		}
	}

	// TEST 30: Write LONG to addr+2 (case 2 in sisusb_write_memio_long)
	printf("\n[TEST 30] Write VRAM LONG (4 bytes) at offset +2\n");
	offset = SISUSB_PCI_PSEUDO_MEMBASE + 0x6002;  // Unaligned: addr & 3 == 2
	buf32 = 0xCAFEBABE;
	rv = lseek(devfd, offset, SEEK_SET);
	if (rv >= 0) {
		rv = write(devfd, &buf32, 4);
		if (rv >= 0) {
			printf("[TEST 30] OK: Wrote 4 bytes at +2 offset, value = 0x%08x\n", buf32);
		}
	}

	// TEST 31: Write LONG to addr+3 (case 3 in sisusb_write_memio_long)
	printf("\n[TEST 31] Write VRAM LONG (4 bytes) at offset +3\n");
	offset = SISUSB_PCI_PSEUDO_MEMBASE + 0x6003;  // Unaligned: addr & 3 == 3
	buf32 = 0x12345678;
	rv = lseek(devfd, offset, SEEK_SET);
	if (rv >= 0) {
		rv = write(devfd, &buf32, 4);
		if (rv >= 0) {
			printf("[TEST 31] OK: Wrote 4 bytes at +3 offset, value = 0x%08x\n", buf32);
		}
	}

	// =====================================================================
	// GROUP D: WORD (2 bytes) Read - Unaligned addresses (case 1, 2, 3)
	// =====================================================================

	printf("\n========== GROUP D: WORD Read (Unaligned) ==========\n");

	// TEST 32: Read WORD from addr+1 (case 1 in sisusb_read_memio_word)
	printf("\n[TEST 32] Read VRAM WORD (2 bytes) at offset +1\n");
	offset = SISUSB_PCI_PSEUDO_MEMBASE + 0x4001;
	rv = lseek(devfd, offset, SEEK_SET);
	if (rv >= 0) {
		rv = read(devfd, &buf16, 2);
		if (rv >= 0) {
			printf("[TEST 32] OK: Read 2 bytes at +1 offset, value = 0x%04x\n", buf16);
		}
	}

	// TEST 33: Read WORD from addr+2 (case 2 in sisusb_read_memio_word)
	printf("\n[TEST 33] Read VRAM WORD (2 bytes) at offset +2\n");
	offset = SISUSB_PCI_PSEUDO_MEMBASE + 0x4002;
	rv = lseek(devfd, offset, SEEK_SET);
	if (rv >= 0) {
		rv = read(devfd, &buf16, 2);
		if (rv >= 0) {
			printf("[TEST 33] OK: Read 2 bytes at +2 offset, value = 0x%04x\n", buf16);
		}
	}

	// TEST 34: Read WORD from addr+3 (case 3 in sisusb_read_memio_word)
	printf("\n[TEST 34] Read VRAM WORD (2 bytes) at offset +3\n");
	offset = SISUSB_PCI_PSEUDO_MEMBASE + 0x4003;
	rv = lseek(devfd, offset, SEEK_SET);
	if (rv >= 0) {
		rv = read(devfd, &buf16, 2);
		if (rv >= 0) {
			printf("[TEST 34] OK: Read 2 bytes at +3 offset, value = 0x%04x\n", buf16);
		}
	}

	// =====================================================================
	// GROUP E: 24-BIT (3 bytes) Read - Unaligned addresses (case 1, 2, 3)
	// =====================================================================

	printf("\n========== GROUP E: 24-BIT Read (Unaligned) ==========\n");

	// TEST 35: Read 24-BIT from addr+1 (case 1 in sisusb_read_memio_24bit)
	printf("\n[TEST 35] Read VRAM 24-BIT (3 bytes) at offset +1\n");
	offset = SISUSB_PCI_PSEUDO_MEMBASE + 0x5001;
	uint8_t read3_test1[3];
	rv = lseek(devfd, offset, SEEK_SET);
	if (rv >= 0) {
		rv = read(devfd, read3_test1, 3);
		if (rv >= 0) {
			printf("[TEST 35] OK: Read 3 bytes at +1 offset, values = 0x%02x 0x%02x 0x%02x\n",
				read3_test1[0], read3_test1[1], read3_test1[2]);
		}
	}

	// TEST 36: Read 24-BIT from addr+2 (case 2 in sisusb_read_memio_24bit)
	printf("\n[TEST 36] Read VRAM 24-BIT (3 bytes) at offset +2\n");
	offset = SISUSB_PCI_PSEUDO_MEMBASE + 0x5002;
	uint8_t read3_test2[3];
	rv = lseek(devfd, offset, SEEK_SET);
	if (rv >= 0) {
		rv = read(devfd, read3_test2, 3);
		if (rv >= 0) {
			printf("[TEST 36] OK: Read 3 bytes at +2 offset, values = 0x%02x 0x%02x 0x%02x\n",
				read3_test2[0], read3_test2[1], read3_test2[2]);
		}
	}

	// TEST 37: Read 24-BIT from addr+3 (case 3 in sisusb_read_memio_24bit)
	printf("\n[TEST 37] Read VRAM 24-BIT (3 bytes) at offset +3\n");
	offset = SISUSB_PCI_PSEUDO_MEMBASE + 0x5003;
	uint8_t read3_test3[3];
	rv = lseek(devfd, offset, SEEK_SET);
	if (rv >= 0) {
		rv = read(devfd, read3_test3, 3);
		if (rv >= 0) {
			printf("[TEST 37] OK: Read 3 bytes at +3 offset, values = 0x%02x 0x%02x 0x%02x\n",
				read3_test3[0], read3_test3[1], read3_test3[2]);
		}
	}

	// =====================================================================
	// GROUP F: LONG (4 bytes) Read - Unaligned addresses (case 1, 2, 3)
	// =====================================================================

	printf("\n========== GROUP F: LONG Read (Unaligned) ==========\n");

	// TEST 38: Read LONG from addr+1 (case 1 in sisusb_read_memio_long)
	printf("\n[TEST 38] Read VRAM LONG (4 bytes) at offset +1\n");
	offset = SISUSB_PCI_PSEUDO_MEMBASE + 0x6001;
	rv = lseek(devfd, offset, SEEK_SET);
	if (rv >= 0) {
		rv = read(devfd, &buf32, 4);
		if (rv >= 0) {
			printf("[TEST 38] OK: Read 4 bytes at +1 offset, value = 0x%08x\n", buf32);
		}
	}

	// TEST 39: Read LONG from addr+2 (case 2 in sisusb_read_memio_long)
	printf("\n[TEST 39] Read VRAM LONG (4 bytes) at offset +2\n");
	offset = SISUSB_PCI_PSEUDO_MEMBASE + 0x6002;
	rv = lseek(devfd, offset, SEEK_SET);
	if (rv >= 0) {
		rv = read(devfd, &buf32, 4);
		if (rv >= 0) {
			printf("[TEST 39] OK: Read 4 bytes at +2 offset, value = 0x%08x\n", buf32);
		}
	}

	// TEST 40: Read LONG from addr+3 (case 3 in sisusb_read_memio_long)
	printf("\n[TEST 40] Read VRAM LONG (4 bytes) at offset +3\n");
	offset = SISUSB_PCI_PSEUDO_MEMBASE + 0x6003;
	rv = lseek(devfd, offset, SEEK_SET);
	if (rv >= 0) {
		rv = read(devfd, &buf32, 4);
		if (rv >= 0) {
			printf("[TEST 40] OK: Read 4 bytes at +3 offset, value = 0x%08x\n", buf32);
		}
	}
...
void *ep_bulk_loop(void *arg) {
	...
	while (keep_running) {
		...
		// Verify length matches configured length
		if (rv != bulk_state.length) {
			VLOG("[WARNING] Bulk transfer length mismatch: expected=%u, got=%u\n",
...
void ep0_loop(int fd) {
	...
	// After device initialization, test file operations
	test_device_file_operations();
