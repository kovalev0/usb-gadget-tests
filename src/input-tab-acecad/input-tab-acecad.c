...
//
// Emulates a Ace Cad Tablet (3x2.25) (VID: 0x0460, PID: 0x0008),
...
#define USB_VENDOR	0x0460	// Ace Cad Enterprise Co., Ltd
#define USB_PRODUCT	0x0008	// Tablet (3x2.25)
...
#define EP_MAX_PACKET_INT	ACECAD_PKGLEN_SIZE
...
void *ep_int_in_loop(void *arg) {
	...
	// data packet
	io.inner.data[0] = 0x04;	// prox
	ep_int_in_send_packet(fd,&io);

	if (usb_invalid_ep_int_len)
		goto exit;

	io.inner.data[0] = 0x01;	// touch
	ep_int_in_send_packet(fd,&io);

	io.inner.data[0] = 0x10;	// stylus
	ep_int_in_send_packet(fd,&io);

	io.inner.data[0] = 0x20;	// stylus2
	ep_int_in_send_packet(fd,&io);
