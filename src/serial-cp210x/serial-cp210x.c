...
//
// Emulates a Silicon Labs CP210x UART Bridge (VID: 0x10c4, PID: 0xea60),
...
// SET_CONFIGURATION) and CP210x-specific vendor requests
// (e.g., GET_PARTNUM, SET_LINE_CTL).
//
...
// The CP2108 USB-to-serial driver has a bug in kernels older
// than 5.11 (e.g., 5.10), where the GET_LINE_CTL register
// value is byte-swapped.
// Introduce default disabled flag.
bool cp210_legacy_line_ctl = false;
...
void log_control_request(struct usb_ctrlrequest *ctrl) {
	...
	case USB_TYPE_VENDOR:
		...
		case CP210X_VENDOR_SPECIFIC:
			printf("  req = CP210X_VENDOR_SPECIFIC\n");
			switch (ctrl->wValue) {
			case CP210X_GET_PARTNUM:
				printf("  vend = CP210X_GET_PARTNUM\n");
				break;
			default:
				printf("  vend = unknown = 0x%x\n",
							ctrl->wValue);
				break;
			}
			break;
		case CP210X_GET_LINE_CTL:
			printf("  req = CP210X_GET_LINE_CTL\n");
			break;
		case CP210X_SET_LINE_CTL:
			printf("  req = CP210X_SET_LINE_CTL\n");
			break;
		default:
			printf("  req = unknown = 0x%x\n", ctrl->bRequest);
			break;
...
#define USB_VENDOR	0x10c4	// Silicon Labs
#define USB_PRODUCT	0xea60	// CP210x UART Bridge
...
bool ep0_request(int fd, struct usb_raw_control_event *event,
	...
	case USB_TYPE_VENDOR:
		...
		case CP210X_VENDOR_SPECIFIC:
			...
			case CP210X_GET_PARTNUM:
				io->data[0] = CP210X_PARTNUM_CP2102;
				io->data[1] = 0;
				io->inner.length = event->ctrl.wLength;

				// Last request ( >= 5.11 )
				if (event->ctrl.wLength == 2 &&
						!cp210_legacy_line_ctl)
		...
		case CP210X_SET_LINE_CTL:
			...
			// Last request ( < 5.11 )
			static int set_lctl_counter = 0;
			if (++set_lctl_counter == 2)
				atomic_store(&ep0_request_end, true);
			return true;
		default:
			printf("fail: no response\n");
...
int main(int argc, char **argv) {
	...
	if (argc >= 2) {
		if (!strcmp(argv[1], "--legacy-line-ctl")) {
			// Enable legacy CP2108 line control bug detection
			printf("--legacy-line-ctl\n");
			cp210_legacy_line_ctl = true;
		}
	}
