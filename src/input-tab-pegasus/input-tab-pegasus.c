...
//
// Emulates a Pegasus Mobile Notetaker Pen input tablet (VID: 0x0e20,
// PID: 0x0101), simulating device enumeration and interrupt In requests.
...
#define PEN_BUTTON_PRESSED	0x2
#define PEN_TIP			0x1

// Use the "--invalid_ep_int_len" option for testing
// usb enumeration with invalid endpoint Int length.
// Introduce default disabled (valid length) flag.
bool usb_invalid_ep_int_len = false;
...
#define USB_VENDOR	0x0e20	// Pegasus Technologies Ltd.
#define USB_PRODUCT	0x0101	// NoteTaker
...
#define EP_MAX_PACKET_INT	NOTETAKER_PKGLEN_SIZE
...
int build_config(char *data, int length, bool other_speed) {
	...
	if (usb_invalid_ep_int_len)
		usb_endpoint.wMaxPacketSize	= 1;
...
bool ep_int_in_button_pressed = false;
...
void *ep_int_in_loop(void *arg) {
	...
	// data packet
	io.inner.data[0] = BATTERY_GOOD;
	io.inner.data[1] = PEN_TIP | PEN_BUTTON_PRESSED;
	ep_int_in_send_packet(fd,&io);

	if (usb_invalid_ep_int_len)
		goto exit;

	io.inner.data[0] = SPECIAL_COMMAND;
	io.inner.data[1] = BUTTON_PRESSED;
	ep_int_in_send_packet(fd,&io);

	// for pegasus->init
	ep_int_in_button_pressed = true;

	io.inner.data[0] = BATTERY_LOW;
	ep_int_in_send_packet(fd,&io);

	io.inner.data[0] = BATTERY_NO_REPORT;
	ep_int_in_send_packet(fd,&io);

	io.inner.data[0] = 0xff;	// unknown answer
	ep_int_in_send_packet(fd,&io);
...
void ep0_loop(int fd) {
	while(true) {
		if (atomic_load(&ep0_request_end)) {
			...
			ep_int_in_button_pressed = false;
		}
