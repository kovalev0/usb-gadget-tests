...
//
// Emulates a LEGO USB Tower device (VID: 0x0694, PID: 0x0001).
...
// SET_CONFIGURATION) and two vendor IN requests:
//   LEGO_USB_TOWER_REQUEST_GET_VERSION (0xFD) called in probe,
//   LEGO_USB_TOWER_REQUEST_RESET (0x04) called in open.
//
// The emulator exercises the legousbtower driver via
// /dev/usb/legousbtower*: open(), read(), write(), poll(), release().
//
...
// Mirrors kernel struct tower_reset_reply
...
// Mirrors kernel struct tower_get_version_reply
...
void log_control_request(struct usb_ctrlrequest *ctrl) {
	...
	case USB_TYPE_VENDOR:
		...
		switch (ctrl->bRequest) {
		case LEGO_USB_TOWER_REQUEST_GET_VERSION:
			printf("  req = GET_VERSION\n");
			break;
		case LEGO_USB_TOWER_REQUEST_RESET:
			printf("  req = RESET\n");
			break;
		default:
			printf("  req = unknown = 0x%x\n", ctrl->bRequest);
...
// Signal from ep_int_out_loop that a write packet was received
...
void *test_thread(void *arg) {
	...
	// tower_open: sends RESET vendor request, submits int_in_urb
	...
	// tower_llseek: always returns -ESPIPE
	off_t off = lseek(devfd, 0, SEEK_SET);
	if (off == (off_t)-1 && errno == ESPIPE)
		printf("[TEST] OK: lseek returned ESPIPE"
			" (expected)\n");
	else
		printf("[TEST] ERR: unexpected lseek result\n");

	// tower_poll: EPOLLIN (data arrived) + EPOLLOUT (out not busy)
	...
	// tower_read: waits for interrupt_in_done, reads from read_buffer
	...
	// tower_write: copies to interrupt_out_buffer, submits OUT URB
	...
	// tower_release: kills URBs, open_count=0
	close(devfd);
...
#define USB_VENDOR		0x0694	// LEGO Group
#define USB_PRODUCT		0x0001	// USB Tower
...
// ep_int_in_loop: continuously sends interrupt IN packets.
// tower_interrupt_in_callback re-submits the URB after each packet.
...
// ep_int_out_loop: receives interrupt OUT packets from tower_write().
...
bool ep0_request(int fd, struct usb_raw_control_event *event,
	...
	case USB_TYPE_STANDARD:
		...
		break;
	case USB_TYPE_VENDOR:
		// Both vendor requests are IN (usb_control_msg_recv).
		switch (event->ctrl.bRequest) {
		case LEGO_USB_TOWER_REQUEST_GET_VERSION: {
			// Called in probe - must respond or probe fails.
			struct tower_get_version_reply reply;
			memset(&reply, 0, sizeof(reply));
			reply.size     = __cpu_to_le16(sizeof(reply));
			reply.err_code = 0;
			reply.major    = 1;
			reply.minor    = 10;
			reply.build_no = __cpu_to_le16(42);
			memcpy(io->data, &reply, sizeof(reply));
			io->inner.length = sizeof(reply);
			return true;
		}
		case LEGO_USB_TOWER_REQUEST_RESET: {
			// Called in tower_open - must respond or open fails.
			struct tower_reset_reply reply;
			memset(&reply, 0, sizeof(reply));
			reply.size     = __cpu_to_le16(sizeof(reply));
			reply.err_code = 0;
			memcpy(io->data, &reply, sizeof(reply));
